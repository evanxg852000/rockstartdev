...
use std::fmt;
use instruction::{Instruction, RawInstrcution};
use display::{Display, SPRITES};
use rand::distributions::{IndependentSample, Range};

const NUM_GERNERAL_PURPOSE_REGS: usize = 16;
...
pub struct Chip8 {
    regs: [u8; NUM_GERNERAL_PURPOSE_REGS],
    i_reg: u16,
    delay_timer_reg: u8,
    sound_timer_reg: u8,
...
impl Chip8 {
    pub fn new(program: Vec<u8>) -> Chip8 {
        ...
        Chip8 {
            regs: [0; NUM_GERNERAL_PURPOSE_REGS],
            i_reg: 0,
            delay_timer_reg: 0,
            sound_timer_reg: 0,
            stack_pointer_reg: 0,
    ...
    fn run_instruction(&self, instruction: &Instrcution) -> u16 {
        match *instruction {
    ...
    fn instruction(&self) ->Instrcution{
        ...
        RawInstrcution::new(higher_order + lower_order)
            .to_instruction()
