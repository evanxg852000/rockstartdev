...
impl KvStore {
    pub fn new(path: impl Into<PathBuf>) -> Result<KvStore> {
        ...
        for &gen in &gen_list {
            let reader = BufWriterWithPos::new(File::open(log_path(&path, gen))?)?;
...
impl KvsEngine for KvStore {
    ...
    fn get(&self, key: String) -> Result<Option<String>> {
        if let cmd_pos = self.index.get(&key) {
            if let Command::Set{value, ..} = self.reader.read_command(*cmd_pos.value())? {
                Ok(Some(value))
            } else {
                Err(KvStore::UnexpectedCommandType)
            }
        } else {
            Ok(None)
        }
