...
fn run(opt: Opt) -> Result<()> {
    ...
    match engine {
        Engine::kvs => run_with(KvStore::open(env::current_dir()?)?, pool, addr),
...
fn run_with<E: KvsEngine, P: ThreadPool>(engine: E, pool: ThreadPool, addr: SocketAddr) -> Result<()> {
...
fn current_engine() -> Result<Option<Engine>> {
    ...
    matche fs::read_to_string(engine)?.parse() {
        Ok(engine) => Ok(Some(engine)),
        Err(e) => {
            warn!("The content of thw engine file is invalid: {}", e);
            Ok(None)
        }
    }
}