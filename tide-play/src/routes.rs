...
pub(crate) async fn find_doc(req: Request<State>) -> tide::Result<impl Into<Response>> {
  ...
  let filter = doc!{"author": "George Orwel"};
  let find_opts = FindOptions::builder().sort(doc!{"title": 1}).build();
  let mut cursor = coll.find(filter, find_opts).await?;
