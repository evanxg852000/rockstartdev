...
// Server wraps a ConsensusModule along with a rpc.Server that exposes
// its methods as RPC endpoints. It also manages the peers of the raft server.
// The main goal of this type is to simplify the code of raft.Server for
// presentation purposes. ConsensusModule has a *Server to do its peer
...
func (s *Server) GetListnerAddr() net.Addr {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *Server) Call(id int, serverMethod string, args interface{}, reply interface{}) error {
	s.mu.Lock()
	peer := s.peerClients[id]
	s.mu.Unlock()

	//If this is after shutdown (where client.CLose is called), it will
	// return an error
	if peer == nil {
		return fmt.Errorf("calk client %d after it's closed", id)
	} else {
...
func (rpp *RPCProxy) AppendEntriesArgs(args AppendEntriesArgs, reply *AppendEntriesReply) error {
	if len(os.Getenv("RAFT_UNRELIABLE_RPC")) > 0 {
		dice := rand.Intn(10)
		if dice == 9 {
