...
impl ActionKV {
    ...
    pub fn insert(&mut self, key: &ByteStr, value: &ByteStr) -> io::Result<()> {
        let position = self.insert_nut_ignore_index(key, value)?;
        self.index.insert(key.to_vec(), position);
        Ok(())
    }

    pub fn insert_nut_ignore_index(&mut self, key: &ByteStr, value: &ByteStr) -> io::Result<u64> {
        ...
        let current_pos = self.f.seek(SeekFrom::Current(0))?;
        f.seek(next_byte)?;
