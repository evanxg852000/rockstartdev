...
pub(crate) fn extract_whitespace(s: &str) -> (&str, &str) {
  take_while(|c| WHITE_SPACE.contains(&c), s)
}

pub(crate) fn extract_whitespace1(s: &str) -> Result<(&str, &str), String> {
  take_while1(|c| WHITE_SPACE.contains(&c), s, "expected whitespace".to_string())
}
...
pub(crate) fn sequence<T>(
  ...
  separator_parse: impl Fn(&str) -> (&str, &str),
  mut s: &str,
...
pub(crate) fn sequence1<T>(
  ...
  let (s, sequence) = sequence(parse, separator_parse, s)?;
...
mod tests {
  ...
  #[test]
  fn do_not_extract_space1_when_input_does_not_starts_with_them() {
    assert_eq!(
