...
impl FuncCall {
  ...
  pub(super) fn eval(&self, env: &Env) -> Result<Val, String> {
    ...
    let (pnames, body) = env.get_func(&self.callee)?;

    let num_expected = pnames.len();
    let num_actual = self.params.len();
    if num_expected != num_actual {
      return Err(format!(
          "expected {} parameters, got {}", num_expected, num_actual
      ));
    }

    for (pname, pexpr) in pnames.into_iter().zip(&self.params) {
      let pval = pexpr.eval(&child_env)?;
      child_env.store_binding(pname, pval);
    }
